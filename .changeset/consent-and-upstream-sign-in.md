...
Transactions are stored in `OAUTH_KV` under `transaction:{sha256(handle)}` with a ten-minute TTL. `denyConsent()` returns the `access_denied` redirect back to the client with its `state` and `iss`. See `docs/consent-page.md` and `docs/upstream-sign-in.md`.
