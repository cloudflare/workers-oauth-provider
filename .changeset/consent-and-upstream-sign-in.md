...
- `beginConsent()` / `approveConsent()` run a consent page whose handle is bound to the browser by a `__Host-` cookie, single-use and valid for ten minutes, with `frame-ancestors 'none'` and `X-Frame-Options: DENY`. `approveConsent({ scope })` takes the scopes the user chose on the page, fewer or more than requested, each in `scopesSupported`.
