...
export const D1_STORAGE_MIGRATIONS: readonly D1StorageMigration[] = Object.freeze([
  Object.freeze({
    ...
    statements: Object.freeze([
      `CREATE TABLE IF NOT EXISTS oauth_storage_schema (namespace TEXT PRIMARY KEY, version INTEGER NOT NULL)`,
      ...
      `CREATE TABLE IF NOT EXISTS oauth_grants (namespace TEXT NOT NULL, user_id TEXT NOT NULL, grant_id TEXT NOT NULL, client_id TEXT NOT NULL, revision INTEGER NOT NULL, created_at INTEGER NOT NULL, expires_at INTEGER, value_json TEXT NOT NULL, PRIMARY KEY(namespace, user_id, grant_id))`,
...
/** Applies the adapter schema using D1's transactional batch primitive. */
export async function migrateD1Storage(database: D1Database): Promise<void> {
  for (const migration of D1_STORAGE_MIGRATIONS) {
    await database.batch(migration.statements.map((sql) => database.prepare(sql)));
  }
}
