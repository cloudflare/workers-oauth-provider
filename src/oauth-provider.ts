...
/**
 * Private cross-package carrier consumed opportunistically by the Agents SDK.
 * Keep this key and record shape in sync with `packages/agents/src/mcp/auth-context.ts`.
 */
...
type MutableExecutionContext = Omit<ExecutionContext, 'props'> & {
  props: any;
  [VERIFIED_OAUTH_CONTEXT]?: VerifiedWorkersOAuthContext;
};
...
/** Installs verified metadata without changing or wrapping the existing `ctx.props` object. */
function installVerifiedOAuthContext(ctx: ExecutionContext, value: VerifiedWorkersOAuthContext): boolean {
  if (VERIFIED_OAUTH_CONTEXT in ctx) return false;
...
class OAuthProviderImpl<Env = Cloudflare.Env> {
  ...
  private async handleApiRequest(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
    ...
    let userId = '';
    let grantId = '';
    let verifiedContext: VerifiedWorkersOAuthContext | undefined;
    ...
    if (tokenData) {
      ...
      let matchedResource: string | undefined;
      if (tokenData.audience) {
        ...
        // Retain the specific audience value that matched this request. The bridge
        // carries one RFC 8707 resource, never an arbitrary array element.
        matchedResource = audiences.find((aud) => audienceMatches(resourceServer, aud));
        if (!matchedResource) {
          return this.createErrorResponse('invalid_token', {
      ...
      // Preserve the existing props contract exactly. The bridge is additive
      // and is installed only for the object-shaped props MCP AuthInfo accepts.
      (ctx as MutableExecutionContext).props = decryptedProps;
      if (isRecord(decryptedProps)) {
        verifiedContext = {
          version: 1,
          token: accessToken,
          clientId: tokenData.grant.clientId,
          scopes: [...(tokenData.scope || tokenData.grant.scope)],
          expiresAt: tokenData.expiresAt,
          ...(matchedResource !== undefined && { resource: matchedResource }),
          props: decryptedProps,
        };
      }
    ...
    // Install the private verified-token carrier immediately before crossing
    // into application code. A duplicate means two authentication layers
    // attempted to establish identity on one request, so fail closed.
    if (verifiedContext && !installVerifiedOAuthContext(ctx, verifiedContext)) {
      return this.createErrorResponse(
        'server_error',
        { description: 'Unable to establish verified request context', statusCode: 500 },
        { category: 'verified_oauth_context', reason: 'duplicate_context' }
      );
    }
