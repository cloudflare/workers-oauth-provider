 ...
 */
type DefaultEnv = typeof env & RequiredEnv;
...
export interface OAuthProviderOptions<Env extends RequiredEnv = DefaultEnv> {
  ...
  ) => Promise<TokenExchangeCallbackResult | undefined | null> | TokenExchangeCallbackResult | undefined | null;
...
class OAuthProviderImpl<Env extends RequiredEnv = DefaultEnv> {
   ...
   */
  private async handleTokenRequest(request: Request, env: Env): Promise<Response> {
 ...
 */
async function encryptProps(data: unknown): Promise<{ encryptedData: string; key: CryptoKey }> {
