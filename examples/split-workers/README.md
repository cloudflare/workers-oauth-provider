...
The [README quick start](../../README.md#quick-start) in full, as two deployable Workers:
...
`e2e.test.ts` runs both in workerd and walks an MCP client from the first `401` to an authorized call (`npx vitest run examples`).
