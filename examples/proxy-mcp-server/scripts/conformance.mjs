...
// It starts `wrangler dev`, registers a client, drives the placeholder login page in
// place of a browser, and reports every check the suite recorded.
//
...
function startWorker(port, inspectorPort) {
  const child = spawn(
    ...
    [
      wranglerCli,
      'dev',
      // The resource identifier must be an HTTPS URL, and the provider compares it
      // against the request URL, so local dev has to serve HTTPS.
      '--local-protocol',
      'https',
      '--port',
      String(port),
  ...
  children.add(child);
  const log = (chunk) => process.env.VERBOSE && process.stdout.write(`[worker] ${chunk}`);
...
/**
 * Wait for three good metadata responses in a row. A Worker that has only just come up
 * can still restart, and one good answer before the CLI starts is not enough.
 */
...
function runConformance(clientId) {
  // The suite has no --resource flag, so its authorization request omits `resource` and
  // the provider binds the grant to its sole registered audience.
  const child = spawn(
    'npx',
    [
      '-y',
