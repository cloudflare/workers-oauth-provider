...
This library can experimentally accept MCP Enterprise-Managed Authorization ID-JAG assertions at the token endpoint. An enterprise IdP issues an ID-JAG JWT to an MCP client, and the MCP client exchanges it for a normal opaque access token issued by this provider.
...
new OAuthProvider({
  ...
  enterpriseManagedAuthorization: {
    ...
    async mapClaims({ claims, requestedScope }) {
      return {
        // The current opaque token format uses ':' as a separator, so mapped user IDs
        // for this flow must not contain ':'. Hash or otherwise encode compound IDs.
...
The mere presence of `enterpriseManagedAuthorization` enables the feature — there is no separate `enabled` flag. `resourceMetadata.resource` is required when EMA is configured (defense-in-depth so the AS never accepts an unbound `resource` claim).

When configured, the authorization server metadata advertises `urn:ietf:params:oauth:grant-type:jwt-bearer`. The token endpoint accepts:
...
Validation includes the ID-JAG `typ` header (`oauth-id-jag+jwt`), trusted issuer, JWKS signature, audience, client binding, resource URI, `exp` / `iat` / `nbf`, maximum assertion lifetime, and `jti` replay protection. Access token TTL is clamped to the assertion lifetime. Refresh tokens are not issued for this grant type.

Deployers may override the default in-memory JWKS cache and KV-backed JTI replay store by supplying custom `jwksProvider` / `jtiStore` adapters — useful for Durable-Object-backed strict-once semantics under concurrent requests.

This support is experimental because the MCP extension is still a draft.
